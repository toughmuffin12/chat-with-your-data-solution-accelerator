...
class CosmosConversationClient:
    def __init__(self):
        ...
        self.enable_message_feedback = True

        # print("AZURE_COSMOS_DB_DATABASE_NAME", self.env_helper.AZURE_COSMOS_DB_DATABASE_NAME)
    ...
    def ensure(self):
        try:
            ...
            print("Error in CosmosConversationClient.ensure", e)
            return False
    ...
    def delete_conversation(self, user_id, conversation_id):
        conversation = self.container_client.read_item(
            item=conversation_id, partition_key=user_id
        )
        if conversation:
            resp = self.container_client.delete_item(
                item=conversation_id, partition_key=user_id
            )
            return resp
        else:
    ...
    def create_message(
        self, uuid, conversation_id, user_id, input_message: dict, metadata
    ):
        message = {
            "id": uuid,
            "type": "message",
    ...
    def update_message_feedback(self, user_id, message_id, feedback):
        message = self.container_client.read_item(
